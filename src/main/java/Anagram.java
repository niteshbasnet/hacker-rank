...
public class Anagram {
	public static void main(String[] args) {
		System.out.println(anagramCheck("cat", "actor"));
	}

	public static boolean anagramCheck(String needle, String haystack) {
		...
		int keySetSize = needleMap.keySet().size();
		char[] h = haystack.toCharArray();
		Map<Character, Integer> haystackMap = new HashMap<Character, Integer>();
		for (int i = 0; i < haystack.length(); i++) {
			if (i >= needle.length()) {
				keySetSize += diff(haystackMap, needleMap, h[i - needle.length()], false);
			}
			keySetSize += diff(haystackMap, needleMap, h[i], true);
			if (keySetSize == 0)
				return true;
		}
		return false;
